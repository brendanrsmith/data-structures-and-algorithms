...
const validateEmail = (email) => {
  // Solution code here...
  // Solution adapted from Steven Smith at RegExLib.com https://regexlib.com/REDetails.aspx?regexp_id=21
  const re = /^([\w]+(\.\w+)?)@([\w-]+\.)[\w-]{3}$/;
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
  // solution adapted from Laurence O'Donnell at RegExLib.com https://regexlib.com/REDetails.aspx?regexp_id=45
  const re = /^(\d{3})\D?\D?(\d{3})\D?(\d{4})$|^\((\d{3})\)\D?(\d{3})\D?(\d{4})$/;
